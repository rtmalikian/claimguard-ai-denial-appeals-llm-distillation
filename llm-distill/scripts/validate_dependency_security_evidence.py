#!/usr/bin/env python3
"""Validate dependency security evidence without storing scan values."""
...
DEFAULT_EVIDENCE = (
    ...
    / "dependency_security_evidence.template.json"
)
DEFAULT_REPORT = DISTILL_DIR / "evals" / "reports" / "dependency_security_evidence_report.json"
DEFAULT_RUNBOOK = DISTILL_DIR / "docs" / "dependency-security-runbook.md"
DEFAULT_PRIVATE_EVIDENCE_RENDERER = (
    DISTILL_DIR / "scripts" / "render_dependency_security_private_evidence.py"
)
EXPECTED_ARTIFACT = "claimguard_dependency_security_evidence"
RUNBOOK_REQUIRED_MARKERS = (
    ...
    "Remediate or privately approve every critical/high finding",
    "Do not store approval reference values",
    "raw vulnerability details",
    "PHIplan production-readiness audit must be rerun after dependency security",
)
PRIVATE_RENDERER_REQUIRED_MARKERS = (
    ...
    "private_dependency_security_summary_checked",
    "private_dependency_security_summary_path_value_included",
    "private_dependency_security_summary_private_reference_count",
    "private_dependency_security_summary_python_package_count",
    "private_dependency_security_summary_frontend_package_count",
    "private_dependency_security_summary_container_image_count",
    "private_dependency_security_summary_remediated_or_approved_finding_count",
    "private_dependency_security_summary_raw_values_included",
    "private dependency security summary private reference count mismatch",
    "unsupported fields",
...
ALLOWED_BOOLEAN_FLAG_KEYS = {
    ...
    "no_raw_scan_output_attested",
    "no_vulnerability_detail_values_attested",
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if (
                ...
                findings.append(
                    f"{child_path}: raw dependency scan, vulnerability, approval, secret, or data value key is not allowed"
                )
...
def evidence_format_requirement(evidence: Any) -> dict[str, Any]:
    ...
    for section_name in ("scan_controls", "remediation_controls", "governance_controls"):
        if not isinstance(evidence.get(section_name), dict):
            blockers.append(f"{section_name} section is required")
    return requirement(
        requirement_id="dependency_security_evidence_format",
        name="Dependency security evidence has the required structure",
...
def no_values_requirement(evidence_path: Path, evidence: Any) -> dict[str, Any]:
    ...
    if phi_findings:
        ...
        blockers.append(f"dependency evidence contains PHI/PII-like metadata findings: {finding_types}")
    ...
    required_attestations = {
        ...
        "no_raw_scan_output_attested": bool_value(evidence, "no_raw_scan_output_attested"),
        "no_vulnerability_detail_values_attested": bool_value(
            evidence,
            "no_vulnerability_detail_values_attested",
        ),
        ...
        "private_dependency_security_summary_path_value_included_false": false_value(
            evidence,
            "private_dependency_security_summary_path_value_included",
        ),
        "private_dependency_security_summary_raw_values_included_false": false_value(
            evidence,
            "private_dependency_security_summary_raw_values_included",
        ),
    }
    ...
    return requirement(
        requirement_id="dependency_security_no_phi_secret_or_values",
        name="Dependency security evidence contains no PHI, secrets, raw scan output, vulnerability detail values, or approval references",
        status="blocked" if blockers else "ready",
        blockers=blockers,
        evidence={
...
def scan_controls_requirement(evidence: dict[str, Any]) -> dict[str, Any]:
    section = evidence.get("scan_controls", {})
    required = {
        "python_dependency_scan_completed": "python_dependency_scan_not_completed",
        "frontend_dependency_scan_completed": "frontend_dependency_scan_not_completed",
        "container_image_scan_completed": "container_image_scan_not_completed",
        "lockfiles_reviewed": "lockfiles_not_reviewed",
        "scan_tools_documented": "scan_tools_not_documented",
    }
    ...
    return requirement(
        ...
        name="Python, frontend, container, and lockfile dependency scans are ready",
        status="blocked" if blockers else "ready",
        blockers=blockers,
...
def remediation_controls_requirement(evidence: dict[str, Any]) -> dict[str, Any]:
    section = evidence.get("remediation_controls", {})
    required = {
        "critical_high_findings_remediated_or_approved": (
            "critical_high_findings_not_remediated_or_approved"
        ),
        "known_vulnerable_packages_reviewed": "known_vulnerable_packages_not_reviewed",
        "compensating_controls_documented": "compensating_controls_not_documented",
        "rebuild_and_retest_completed": "rebuild_and_retest_not_completed",
        "upgrade_plan_documented": "upgrade_plan_not_documented",
    }
    blockers = [blocker for key, blocker in required.items() if not bool_value(section, key)]
    return requirement(
        requirement_id="dependency_security_remediation_controls",
        name="Dependency findings are remediated, approved, documented, rebuilt, and retested",
        status="blocked" if blockers else "ready",
        blockers=blockers,
        evidence={key: bool_value(section, key) for key in required} | {"values_redacted": True},
    )
...
def private_summary_requirement(evidence: dict[str, Any]) -> dict[str, Any]:
    required_positive_counts = {
        "private_dependency_security_summary_private_reference_count": (
            "private_dependency_security_summary_private_reference_count_missing"
        ),
        "private_dependency_security_summary_python_package_count": (
            "private_dependency_security_summary_python_package_count_missing"
        ),
        "private_dependency_security_summary_frontend_package_count": (
            "private_dependency_security_summary_frontend_package_count_missing"
        ),
        "private_dependency_security_summary_container_image_count": (
            "private_dependency_security_summary_container_image_count_missing"
        ),
    }
    blockers: list[str] = []
    if not bool_value(evidence, "private_dependency_security_summary_path_configured"):
        blockers.append("private_dependency_security_summary_path_not_configured")
    if not bool_value(evidence, "private_dependency_security_summary_checked"):
        blockers.append("private_dependency_security_summary_not_checked")
    if not false_value(evidence, "private_dependency_security_summary_path_value_included"):
        blockers.append("private_dependency_security_summary_path_value_included")
    if not false_value(evidence, "private_dependency_security_summary_raw_values_included"):
        blockers.append("private_dependency_security_summary_raw_values_included")
    ...
    return requirement(
        ...
        name="Private dependency security summary metadata is present without raw values",
        status="blocked" if blockers else "ready",
        blockers=blockers,
        evidence={
            "private_dependency_security_summary_path_env": evidence.get(
                "private_dependency_security_summary_path_env"
            ),
            "private_dependency_security_summary_path_configured": bool_value(
                evidence,
                "private_dependency_security_summary_path_configured",
            ),
            "private_dependency_security_summary_path_value_included": bool_value(
                evidence,
                "private_dependency_security_summary_path_value_included",
            ),
            "private_dependency_security_summary_checked": bool_value(
                evidence,
                "private_dependency_security_summary_checked",
            ),
            **{key: evidence.get(key) for key in required_positive_counts},
            "private_dependency_security_summary_remediated_or_approved_finding_count": (
                evidence.get("private_dependency_security_summary_remediated_or_approved_finding_count")
            ),
            "values_redacted": True,
        },
    )
...
def source_control_runbook_requirement(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    return requirement(
        requirement_id="dependency_security_runbook",
        name="Source-controlled dependency security runbook has required safety markers",
        status="blocked" if blockers else "ready",
        blockers=blockers,
        evidence={
...
def private_renderer_requirement(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    return requirement(
        requirement_id="dependency_security_private_evidence_renderer",
        name="Private dependency security evidence renderer exists with no-source-control and redaction markers",
        status="blocked" if blockers else "ready",
        blockers=blockers,
        evidence={
...
def governance_controls_requirement(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    return requirement(
        ...
        name="Dependency security approval and metadata-only audit governance controls are ready",
        status="blocked" if blockers else "ready",
        blockers=blockers,
        evidence={key: bool_value(section, key) for key in required} | {"values_redacted": True},
    )
...
def build_report(evidence_path: Path = DEFAULT_EVIDENCE) -> dict[str, Any]:
    ...
    requirements = [
        ...
        scan_controls_requirement(evidence),
        remediation_controls_requirement(evidence),
    ...
    if load_errors:
        requirements.insert(
            0,
            requirement(
                requirement_id="dependency_security_evidence_load",
                name="Dependency security evidence file can be loaded",
                status="blocked",
    ...
    dependency_security_ready = not blocked_items and bool_value(
        evidence,
        "dependency_security_ready",
    )
    ready_flag = requirement(
        requirement_id="dependency_security_ready_flag",
        name="Dependency security evidence is explicitly marked ready",
        status="ready" if bool_value(evidence, "dependency_security_ready") else "blocked",
        blockers=[] if bool_value(evidence, "dependency_security_ready") else [
            "dependency_security_ready is not true"
        ],
        evidence={
            "dependency_security_ready": bool_value(evidence, "dependency_security_ready"),
            "values_redacted": True,
        },
    )
    ...
    return {
        "artifact": "dependency_security_evidence_validation_report",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "safe_to_review": not any(
            item["requirement_id"] == "dependency_security_no_phi_secret_or_values"
            and item["status"] == "blocked"
            for item in requirements
        ),
        "dependency_security_ready": dependency_security_ready,
        "blocked_item_count": len(blocked_items),
        "blocked_items": blocked_items,
        "requirements": requirements,
        "evidence": {
            "evidence_path": sanitize_report_value(str(evidence_path), REPO_ROOT),
            "runbook": "llm-distill/docs/dependency-security-runbook.md",
...
def main() -> int:
    ...
    print(
        f"wrote dependency security evidence report to {args.report} "
        f"ready={report['dependency_security_ready']} "
