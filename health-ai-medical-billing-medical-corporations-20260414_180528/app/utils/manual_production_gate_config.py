...
def _blocked_requirement_ids_from_items(
    items: Any,
    *,
    ...
    for item in items:
        ...
        if status != "blocked" and not (allow_missing_status and status is None):
            continue
...
def _blocked_requirement_ids(report: dict[str, Any] | None) -> list[str]:
    ...
    requirement_ids = _blocked_requirement_ids_from_items(requirements)
    if requirement_ids:
        return requirement_ids
    return _blocked_requirement_ids_from_items(
        report.get("blocked_items"),
        allow_missing_status=True,
    )


def validate_manual_production_gate_startup_config(
    *,
    settings_like=None,
    ...
    report_ready = (
        evidence_report.get("production_gate_ready") is True
    ...
    if not evidence_report_configured:
        blockers.append("manual_production_gate_packet_report_path_missing")
    elif not evidence_report_checked:
        blockers.append("manual_production_gate_packet_report_unavailable")
    if evidence_report_checked and report_ready is not True:
        blockers.append("manual_production_gate_packet_report_not_ready")
    if evidence_report_checked and report_safe_to_review is not True:
        blockers.append("manual_production_gate_packet_report_not_safe_to_review")
    if blocked_requirement_ids:
        blockers.append("manual_production_gate_packet_report_has_blocked_requirements")

    safe_context = {
        "evidence_report_path_included": False,
        "raw_evidence_report_included": False,
        "approval_reference_value_included": False,
        "manifest_record_ids_included": False,
        "private_summary_path_included": False,
    ...
    if blockers:
        logger.warning(
            "manual_production_gate_startup_config_validation_failed",
            extra={"manual_production_gate_startup_config": log_payload},
        )
    else:
        logger.info(
            "manual_production_gate_startup_config_validation_passed",
            extra={"manual_production_gate_startup_config": log_payload},
        )
    if report["fail_fast_required"]:
        raise RuntimeError("Manual production gate startup configuration is not ready.")
    return report
