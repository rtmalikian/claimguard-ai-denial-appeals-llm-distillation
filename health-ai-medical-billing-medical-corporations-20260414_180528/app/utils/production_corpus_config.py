...
def validate_production_corpus_startup_config(
    *,
    settings_like=None,
    ...
    report_ready = (
        evidence_report.get("production_corpus_ready") is True
    ...
    if not evidence_report_configured:
        blockers.append("production_corpus_evidence_report_path_missing")
    elif not evidence_report_checked:
        blockers.append("production_corpus_evidence_report_unavailable")
    ...
    safe_context = {
        ...
        "raw_checksum_included": False,
        "private_manifest_path_included": False,
    ...
    if blockers:
        logger.warning(
            "production_corpus_startup_config_validation_failed",
            extra={"production_corpus_startup_config": log_payload},
        )
    else:
        logger.info(
            "production_corpus_startup_config_validation_passed",
            extra={"production_corpus_startup_config": log_payload},
        )
    if report["fail_fast_required"]:
        raise RuntimeError("Production corpus startup configuration is not ready.")
    return report
