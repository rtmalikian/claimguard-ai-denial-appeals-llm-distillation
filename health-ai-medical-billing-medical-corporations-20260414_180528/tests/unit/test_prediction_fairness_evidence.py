...
def test_template_is_safe_to_review_but_not_production_ready():
    ...
    governance_requirement = _requirement(
        report,
        "prediction_fairness_governance_controls",
    )
    runbook_requirement = _requirement(
        report,
        "prediction_fairness_monitoring_runbook",
    )
    calibration_checklist_requirement = _requirement(
        report,
        "prediction_fairness_calibration_checklist",
    )
    monitoring_validation_checklist_requirement = _requirement(
        report,
        "prediction_fairness_monitoring_validation_checklist",
    )
    legal_privacy_checklist_requirement = _requirement(
        report,
        "prediction_fairness_legal_privacy_checklist",
    )
    private_renderer_requirement = _requirement(
        report,
        "prediction_fairness_private_evidence_renderer",
    )
    ...
    assert governance_requirement["evidence"]["model_card_inside_source_control"] is True
    ...
    assert (
        runbook_requirement["evidence"]["monitoring_runbook_inside_source_control"]
        is True
    )
...
def test_model_card_document_markers_are_required_when_attested(tmp_path, monkeypatch):
    validator = _load_validator()
    ...
    _allow_tmp_source_control_path(monkeypatch, validator, model_card_path)
    evidence = _ready_evidence()
...
def test_monitoring_runbook_markers_are_required_when_documented(
    tmp_path,
    monkeypatch,
):
    validator = _load_validator()
...
def test_calibration_checklist_markers_are_required_when_documented(
    tmp_path,
    monkeypatch,
):
    validator = _load_validator()
...
def test_monitoring_validation_checklist_markers_are_required_when_documented(
    tmp_path,
    monkeypatch,
):
    validator = _load_validator()
...
def test_legal_privacy_checklist_markers_are_required_when_documented(
    tmp_path,
    monkeypatch,
):
    validator = _load_validator()
...
def test_private_evidence_renderer_markers_are_required_when_documented(
    tmp_path,
    monkeypatch,
):
    validator = _load_validator()
...
def test_model_card_must_stay_inside_source_control(tmp_path):
    validator = _load_validator()
    evidence_path = tmp_path / "fairness_outside_model_card.json"
    model_card_path = tmp_path / "prediction-fairness-model-card.md"
    unique_text = "outside prediction fairness model card marker should not leak"
    _write_marker_file(
        model_card_path,
        validator.MODEL_CARD_REQUIRED_MARKERS,
        unique_text,
    )
    evidence = _ready_evidence()
    evidence["governance_controls"]["model_card_path"] = str(model_card_path)
    ...
    assert report["prediction_fairness_monitoring_ready"] is False
    assert (
        "source_control_model_card_must_be_inside_repo"
        in governance_requirement["blockers"]
    )
    assert "model_card_required_markers_missing" not in governance_requirement["blockers"]
    assert governance_requirement["evidence"]["model_card_exists"] is True
    assert governance_requirement["evidence"]["model_card_inside_source_control"] is False
    assert governance_requirement["evidence"]["model_card_present_marker_count"] == 0
    assert governance_requirement["evidence"]["model_card_values_included"] is False
    assert unique_text not in serialized


def test_monitoring_runbook_must_stay_inside_source_control(tmp_path):
    validator = _load_validator()
    evidence_path = tmp_path / "fairness_outside_runbook.json"
    runbook_path = tmp_path / "prediction-fairness-monitoring-runbook.md"
    unique_text = "outside prediction fairness runbook marker should not leak"
    ...
    assert report["prediction_fairness_monitoring_ready"] is False
    assert (
        "source_control_monitoring_runbook_must_be_inside_repo"
        in runbook_requirement["blockers"]
    )
    assert (
        "monitoring_runbook_required_markers_missing"
    ...
    assert runbook_requirement["evidence"]["monitoring_runbook_present_marker_count"] == 0
    assert runbook_requirement["evidence"]["monitoring_runbook_values_included"] is False
    assert unique_text not in serialized


def test_calibration_checklist_must_stay_inside_source_control(tmp_path):
    validator = _load_validator()
    evidence_path = tmp_path / "fairness_outside_calibration_checklist.json"
    checklist_path = tmp_path / "prediction-fairness-calibration-checklist.md"
    unique_text = "outside calibration checklist marker should not leak"
    _write_marker_file(
        checklist_path,
        validator.CALIBRATION_CHECKLIST_REQUIRED_MARKERS,
        unique_text,
    )
    evidence = _ready_evidence()
    evidence["calibrated_threshold"]["calibration_checklist_path"] = str(
    ...
    assert report["prediction_fairness_monitoring_ready"] is False
    assert (
        "source_control_calibration_checklist_must_be_inside_repo"
        in checklist_requirement["blockers"]
    )
    assert (
        "calibration_checklist_required_markers_missing"
    ...
    assert (
        checklist_requirement["evidence"]["calibration_checklist_present_marker_count"]
        == 0
    )
    assert checklist_requirement["evidence"]["calibration_checklist_values_included"] is False
    assert unique_text not in serialized


def test_monitoring_validation_checklist_must_stay_inside_source_control(tmp_path):
    validator = _load_validator()
    evidence_path = tmp_path / "fairness_outside_monitoring_validation.json"
    checklist_path = tmp_path / "prediction-fairness-monitoring-validation-checklist.md"
    unique_text = "outside monitoring validation checklist marker should not leak"
    _write_marker_file(
        checklist_path,
        validator.MONITORING_VALIDATION_CHECKLIST_REQUIRED_MARKERS,
        unique_text,
    )
    evidence = _ready_evidence()
    evidence["fairness_monitoring"]["monitoring_validation_checklist_path"] = str(
    ...
    assert (
        checklist_requirement["evidence"]["monitoring_validation_checklist_exists"]
        is True
    )
    assert (
        checklist_requirement["evidence"][
    ...
    assert (
        checklist_requirement["evidence"][
            "monitoring_validation_checklist_present_marker_count"
        ]
        == 0
    )
    assert (
...
def test_legal_privacy_checklist_must_stay_inside_source_control(tmp_path):
    validator = _load_validator()
    evidence_path = tmp_path / "fairness_outside_legal_privacy_checklist.json"
    checklist_path = tmp_path / "prediction-fairness-legal-privacy-checklist.md"
    unique_text = "outside legal privacy checklist marker should not leak"
    _write_marker_file(
        checklist_path,
        validator.LEGAL_PRIVACY_CHECKLIST_REQUIRED_MARKERS,
        unique_text,
    )
    evidence = _ready_evidence()
    evidence["governance_controls"]["legal_privacy_checklist_path"] = str(
    ...
    assert report["prediction_fairness_monitoring_ready"] is False
    assert (
        "source_control_legal_privacy_checklist_must_be_inside_repo"
        in checklist_requirement["blockers"]
    )
    assert (
        "legal_privacy_checklist_required_markers_missing"
    ...
    assert checklist_requirement["evidence"]["legal_privacy_checklist_exists"] is True
    assert (
        checklist_requirement["evidence"][
            "legal_privacy_checklist_inside_source_control"
        ]
    ...
    assert (
        checklist_requirement["evidence"][
            "legal_privacy_checklist_present_marker_count"
        ]
        == 0
    )
    assert checklist_requirement["evidence"]["legal_privacy_checklist_values_included"] is False
    assert unique_text not in serialized


def test_private_evidence_renderer_must_stay_inside_source_control(tmp_path):
    validator = _load_validator()
    evidence_path = tmp_path / "fairness_outside_private_renderer.json"
    renderer_path = tmp_path / "render_prediction_fairness_private_evidence.py"
    unique_text = "outside prediction fairness private renderer marker should not leak"
    _write_marker_file(
        renderer_path,
    ...
    assert (
        renderer_requirement["evidence"][
            "private_evidence_renderer_present_marker_count"
        ]
        == 0
    )
    assert renderer_requirement["evidence"]["raw_renderer_text_included"] is False
